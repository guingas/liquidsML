...
CLASSIFICADORES = {
    ...
    "cerveja_estilo":     {"nome": "Estilo (Cerveja)",     "target": "subtipo",            "filtro": "cerveja"},
}
...
def preparar_todos_datasets(df_completo):
    """Prepara os 4 datasets (um por classificador) a partir do dataset completo."""
    datasets = {}

    # 1. Tipo (dataset completo)
    ...
    datasets["cerveja_estilo"] = preprocessar(df_cerveja, "subtipo")
...
def exibir_resultado_pipeline(resultado, nome_modelo):
    ...
    if tipo == "agua":
        ...
        col1.metric("1️⃣ Tipo", f"💧 {tipo.upper()}", f"{conf_tipo:.1f}%")
        ...
        st.metric("1️⃣ Tipo", tipo.upper(), f"{conf_tipo:.1f}%")
...
```
         ...
         └──┬────┬───┘   └──────┬───────┘
            ▼    ▼              ▼
     ┌──────────┐ ┌─────────┐ ┌──────────────┐
     │Potável?  │ │Variante │ │ Estilo:      │
     │potável / │ │torneira/│ │ heineken/    │
     │contamin. │ │mineral/ │ │ budweiser/   │
     └──────────┘ │min.gás/ │ │ ipa/stout    │
                  │contam.  │ └──────────────┘
                  └─────────┘
```
    ...
    with tab_treino:
        ...
        st.markdown(
            f"Treina **{modelo_escolhido}** para os **4 classificadores** do pipeline de uma vez."
        )
        ...
        if st.button("🚀 Treinar Pipeline Inteiro", key="btn_treinar", width="stretch"):
            ...
            for i, (clf_key, clf_info) in enumerate(CLASSIFICADORES.items()):
                ...
                cols[i].metric(clf_info["nome"], f"{acc*100:.1f}%")
...
def _render_tab_ablation(df_completo, modelo_escolhido):
    ...
    if st.button("🚀 Treinar com sensores selecionados", key="btn_ablation"):
        ...
        for idx, clf_key in enumerate(classificadores_ablacao):
            ...
            colunas_excluir = {"tipo", "subtipo", "potabilidade", "potabilidade_label", target_col}
            feature_names = [
